...
- **`document.author` and `footer.author`** — the document's owner, set in the
  ...
  `document.author` out, set `footer.author` to `Prepared by the design
  owner`, and sign any change-record row you write as `the design owner` —
  the owner signs all of it later.
