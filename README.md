...
| 🧭 **Declared scope** | Five opt-in coverage packs — management & OOB, wireless, fabric overlay, WAN / multi-site, security & segmentation — and five declarable architectures (collapsed core, three-tier, spine-leaf, hub-and-spoke, ring). The document promises — as a **Declared scope** row on its own cover, on screen and in print — and the engineering review holds it to the promise: the helper's scope report scores every declared pack **open** (with the question and its fix prompt) or **quiet**, right where the declaration is made. |
